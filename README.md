...
Тестирование проводилось с помощью [k6](https://k6.io/) против production-кластера (`auto-platfrom.ru`).

**Сценарий:** плавный разгон от 50 до 1000 виртуальных пользователей за 4.5 минуты.

**Скрипт:** [`load_test.js`](load_test.js)
...
| Метрика | Значение |
|---|---|
| Всего запросов | 281 883 |
| Пиковый RPS | ~1043 req/s |
| p95 latency | 872ms |
| Ошибки при 1000 VU | 100% (`connection reset by peer`) |

### Вывод

Система уверенно держит нагрузку до **~500 VU**. При 1000 VU nginx начинает сбрасывать TCP-соединения — исчерпывается очередь worker connections.

**Узкие места (инфраструктура, не код):**
- CPU лимиты подов: 250m × 3 реплики = 0.75 ядра на сервис
- PostgreSQL без connection pooler (PgBouncer) — пул соединений переполняется
- nginx worker connections по умолчанию

**Что нужно для масштабирования:**
- HPA (горизонтальный автоскейлинг по CPU)
- PgBouncer перед PostgreSQL
- Увеличение CPU лимитов и числа реплик
